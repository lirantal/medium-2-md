  ...
  replacement: function (content) {
    ...
    if (downloadImages === true && element) {
      ...
      if (utils.isUrl(imgSrc)) {
        ...
        if (imgPathAlias) {
          element = "![](" + path.join(imgPathAlias, imgFileName) + ")";
        } else {
...
const convert = function (htmlStr, downloadImagesFlag, imagesPath, imagesPathAlias = undefined) {
  downloadImages = downloadImagesFlag;
  images = [];
  imgPath = imagesPath;
  imgPathAlias = imagesPathAlias;
