  ...
  replacement: function (content) {
    ...
    if (downloadImages === true && element) {
      ...
      if (utils.isUrl(imgSrc)) {
        ...
        let localImgPath = path.join(imgPath, imgFileName);
        
        if (imgPathAlias) {
          localImgPath = path.join(imgPathAlias, imgFileName);
...
const convert = function (htmlStr, downloadImagesFlag, imagesPath, optionsImgPathAlias) {
  downloadImages = downloadImagesFlag;
  images = [];
  imgPath = imagesPath;
  imgPathAlias = optionsImgPathAlias;
