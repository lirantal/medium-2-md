...
var processAll = async function (inputDir, options) {
  try {
    ...
    if (fs.existsSync(inputPath)) {
      ...
      const paths = fs.readdirSync(inputPath)
      for (const file of paths) {
        try {
          if (path.parse(file).ext === ".html") {
            if (checkDraft(file, options.drafts)) {
              const readOutput = await read.readAll(
                path.join(inputDir, file),
                options.frontMatter,
                imgDir,
                options.imgPathFeatured
              );
              const converterResult = convert(readOutput.html, options.images, imgDir, options.imgPathAlias);
              if (options.images) {
