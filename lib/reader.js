...
var readAll = async function (filePath, frontMatterConfig, imgDir, imgPathAlias) {
  ...
  const frontMatter = await generateFrontMatter(title, subtitle, date, slug, tags, imgDir, imgPathAlias);
...
var generateFrontMatter = async function (title, subtitle, date, slug, tags, imgDir, imgPathAlias) {
  ...
  // for each article
  const randomImageUrl = 'https://source.unsplash.com/random/1600x900/?colorful%20space';
  const randomImageName = `${randomBytes(16).toString('hex')}.jpeg`;
  const randomImageLocalPath = path.join(imgDir, randomImageName);

  await downloader(randomImageUrl, randomImageLocalPath);

  if (imgPathAlias) {
    frontMatter.image = path.join(imgPathAlias, randomImageName)
  } else {
    frontMatter.image = randomImageLocalPath
  }
