...
var readAll = async function (filePath, frontMatterConfig, imgDir) {
  ...
  const frontMatter = await generateFrontMatter(title, subtitle, date, slug, tags, imgDir);
...
var generateFrontMatter = async function (title, subtitle, date, slug, tags, imgDir) {
  ...
  const randomImageLocalPath = path.join(imgDir, `${randomImageName}.jpeg`);
